...
map(
  "i",
  "<C-\\>",
  "<C-o><CMD>set paste<CR><C-r>+<C-o><CMD>set nopaste<CR>",
  opts
)
...
map(
  "n",
  "<leader><leader>",
  '<CMD>lua require("plugins/util").telescope_find_files()<CR>',
  opts
)
map(
  "n",
  "<leader>bb",
  '<CMD>lua require("plugins/util").telescope_buffers()<CR>',
  opts
)
map(
  "n",
  "<leader>sp",
  '<CMD>lua require("plugins/util").telescope_live_grep()<CR>',
  opts
)
map(
  "v",
...
map(
  "n",
  "<leader>sP",
  '<CMD>lua require("plugins/util").telescope_live_grep_in_folder()<CR>',
  opts
)
map(
  "v",
...
map(
  "n",
  "<leader>hh",
  '<CMD>lua require("plugins/util").telescope_help_tags()<CR>',
  opts
)
map(
  "n",
  "<leader>pp",
  '<CMD>lua require("plugins/util").telescope_project()<CR>',
  opts
)
map(
  "n",
  "<leader>rf",
  '<CMD>lua require("plugins/util").telescope_recent()<CR>',
  opts
)
-- map(
--   "n",
--   "-",
--   '<CMD>lua require("oil").open({desc = "Open parent directory"})<CR>',
--   opts
-- )
vim.keymap.set(
  "n",
...
map(
  "n",
  "<leader>iX",
  "<C-w><RIGHT><CMD>bdelete!<CR>A<C-c><C-c><CMD>sleep 100m<CR><C-c>",
  opts
)
...
map(
  "n",
  "<leader>z",
  "<CMD>lua require('zen-mode').toggle({ window = { width = 0.35 } })<CR>",
  opts
)
...
map(
  "n",
  "<leader>V",
  "<CMD>bdelete<CR><CMD>bdelete<CR><CMD>bdelete<CR><CMD>bdelete<CR>",
  opts
)
